...
module.exports.userHasAllRoles = (...roles) => async (req, res, next) => {
	try {
		...
		if (!(await Auth.userHasAllRoles(req.userId, roles))) {
...
module.exports.userHasAnyRoles = (...roles) => async (req, res, next) => {
	try {
		...
		if (!(await Auth.userHasAnyRoles(req.userId, roles))) {
			throw notHasRolesError;
		}
		return next();
	} catch (error) {
...
class Controller {
	...
	mount(app, path) {
		app.use(path, express.json({ limit: '50mb' }), express.urlencoded({ extended: false }), this.router);
	}
}
